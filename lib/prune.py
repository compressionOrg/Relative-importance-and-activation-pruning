...
def prune_magnitude(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    ...
    for i in range(len(layers)):
        ...
        for name in subset:
            ...
            if args.prune_method == "magnitude":
            ...
            elif args.prune_method == "mentropy":
                W_metric = torch.abs(W)
                epsilon = 1e-10
                W_metric += epsilon
                # 归一化张量使其元素和为1
                ...
                W_metric = torch.abs(W_metric * H).reshape(-1, 1)
...
def prune_entropy(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    ...
    for i in range(len(layers)):
        ...
        for name in subset:
            ...
            elif args.prune_method == "zscore":  
                
                # st() 
                W_mean = torch.mean(W)
                W_std = torch.std(W)
                z_scores = (W - W_mean) / W_std
                W_metric = (torch.sum(z_scores > 1) / W.numel()) *torch.abs(W) * torch.sqrt(wrapped_layers[name].scaler_row.reshape((1,-1)))
...
def prune_semi(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    use_cache = model.config.use_cache 
    model.config.use_cache = False 
    print("loading calibdation data")
    dataloader, _ = get_loaders(args.calib_dataset,nsamples=args.nsamples,seed=args.seed,seqlen=args.seqlen,tokenizer=tokenizer)
            ...
            if prune_n != 0:
                ...
                for ii in range(W_metric.shape[1]):
                    if ii % prune_m == 0:
                        tmp = W_metric[:, ii:(ii+prune_m)].float()
                        # 计算信息熵
                        # pr = torch.abs(tmp)/torch.sum(torch.abs(tmp), dim=0)
                        # pc = torch.abs(tmp)/torch.sum(torch.abs(tmp), dim=1).reshape(-1, 1)
                        # entropy = torch.abs((-pr * torch.log(pr)) - (pc * torch.log(pc)))
                        pr = torch.abs(tmp) / torch.sum(torch.abs(tmp))
                        entropy = -pr * torch.log(pr)
